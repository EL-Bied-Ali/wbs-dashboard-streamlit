...
_icon_path = ROOT / "Chronoplan_ico.png"
st.set_page_config(
    ...
    <div class="table-card compact">
      <div class="table-wrap">
        <table class="neo">
          <thead><tr>
            <th></th>
            <th{_html_title_attr(TOOLTIPS.get("planned_finish"))}>Planned</th>
            <th{_html_title_attr(TOOLTIPS.get("forecast_finish"))}>Forecast</th>
            <th{_html_title_attr(TOOLTIPS.get("schedule"))}>Schedule</th>
            <th{_html_title_attr(TOOLTIPS.get("earned"))}>Earned</th>
            <th{_html_title_attr(TOOLTIPS.get("variance"))}>+Variance</th>
            <th{_html_title_attr(TOOLTIPS.get("impact"))}>Impact</th>
            <th{_html_title_attr(TOOLTIPS.get("glissement"))}>glissement</th>
          </tr></thead>
        ...
        if show_summary_chart and depth >= 1:
            with st.container(key=f"{base}__chartbar"):
                ...
                with col_b:
                    ...
                    if st.button(
                        "Show / Hide",
                        key=f"{base}__hide_chart",
                        help=TOOLTIPS["show_hide"],
                    ):
