...
def owner_id_from_user(user: dict | None) -> str | None:
    ...
    # Canonical owner identifier (backward compatible with existing projects.json)
    sub = (user.get("sub") or "").strip()
    if sub:
        return f"acct:sub:{sub}"

    # Fallback to billing account id (newer system)
...
def get_project(project_id: str | None, owner_id: str | int | None = None) -> dict | None:
    ...
    owner_key = _normalize_owner_id(owner_id)
    if not owner_key:
        return None  # DENY if owner_id missing
    projects = _load_projects()
...
def delete_project(
    ...
    *,
    ...
    owner_key = _normalize_owner_id(owner_id)
    if not owner_key:
        return False  # DENY if owner_id missing
...
def store_project_upload(project: dict | None, uploaded, user: dict | None = None) -> str | None:
    ...
    if uploaded is None:
        return st.session_state.get("shared_excel_path")
    owner_key = owner_id_from_user(user)
    if not owner_key:
        owner_key = _normalize_owner_id(project.get("owner_id"))
...
def persist_project_mapping(project_id: str | None, mapping: dict, mapping_key: str | None, user: dict | None = None) -> None:
    ...
    if not owner_key:
        ...
        if idx is None:
            return
        owner_key = _normalize_owner_id(projects[idx].get("owner_id"))
    if not owner_key:
        return
    update_project(project_id, owner_id=owner_key, mapping=mapping, mapping_key=mapping_key, user=user)
