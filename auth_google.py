...
_DEV_USERS_LIMIT = 20
_PENDING_REF_BY_EMAIL: dict[str, tuple[float, str]] = {}
_PENDING_REF_TTL_SECONDS = 3600
...
def switch_dev_user(email: str, name: str | None = None, ref_code: str | None = None) -> None:
    ...
    for k in list(st.session_state.keys()):
        if isinstance(k, str) and k.startswith("_post_login_guard::"):
...
def _expire_cookie_js(name: str, key: str | None = None) -> None:
    key = key or f"expire_cookie_js_{name}_{int(time.time()*1000)}"
...
def _load_user_from_request_cookie(cfg: dict[str, Any]) -> dict[str, Any] | None:
    if not _use_auth_cookies():
        _debug_log("cookie_load skipped (cookies disabled)")
        return None
...
_POST_LOGIN_CLAIM_TTL_SECONDS = 30


def _post_login_guard_key(email: str) -> str:
    return f"_post_login_guard::{(email or '').strip().lower()}"


def _post_login_already_done(email: str) -> bool:
    key = _post_login_guard_key(email)
    entry = st.session_state.get(key)
    return isinstance(entry, dict) and entry.get("state") == "done"


def _post_login_try_claim(email: str) -> bool:
    key = _post_login_guard_key(email)
    now = time.time()
    entry = st.session_state.get(key)
    ...
    if isinstance(entry, dict):
        ...
        if state == "done":
            return False
        if state == "running" and isinstance(ts, (int, float)) and (now - ts) < _POST_LOGIN_CLAIM_TTL_SECONDS:
            return False

    st.session_state[key] = {"state": "running", "ts": now}
    return True


def _post_login_mark_done(email: str) -> None:
    key = _post_login_guard_key(email)
    st.session_state[key] = {"state": "done", "ts": time.time()}
...
def _stash_referral_code(params: dict[str, Any]) -> None:
    ...
    print("[AUTH] stash_referral: set _pending_ref =", code)
    _pending_ref_global_set(code)
    print("[AUTH] stash_referral: saved global pending ref =", code)
    try:
        _inject_cookie_js(REFERRAL_COOKIE, code, 3600, key=f"referral_cookie_js_{int(time.time()*1000)}")
        print("[AUTH] stash_referral: wrote cookie referral_code =", code)
    except Exception as e:
        print("[AUTH] stash_referral: cookie js failed:", e)
...
def _consume_referral_code(params: dict[str, Any], email: str | None = None) -> str | None:
    _cleanup_pending_ref_email_store()
    print(
        "[AUTH] consume_referral: session _pending_ref =",
        st.session_state.get("_pending_ref"),
    )
    ...
    cookies = _get_cookie_manager(refresh=True) if _use_auth_cookies() else None
    if not code:
        pending = st.session_state.get("_pending_ref")
        code = (pending or "").strip() if isinstance(pending, str) else ""
    if not code and email:
        key = email.strip().lower()
        item = _PENDING_REF_BY_EMAIL.get(key)
        if item:
            ts, ref_val = item
            if time.time() - ts <= _PENDING_REF_TTL_SECONDS:
                code = ref_val
                print("[AUTH] consume_referral: got from email store =", code)
            _PENDING_REF_BY_EMAIL.pop(key, None)
    if not code:
        headers = _request_headers()
        raw_cookie = headers.get("cookie") or headers.get("Cookie") or ""
        if raw_cookie:
            vals = _cookie_header_values(raw_cookie, REFERRAL_COOKIE)
            if vals:
                code = unquote(vals[-1]).strip()
                print("[AUTH] consume_referral: got from header cookie =", code)
    if not code:
    ...
    if code:
        ...
        if cookies is not None:
            try:
                ...
                pass
        try:
            _expire_cookie_js(REFERRAL_COOKIE)
            print("[AUTH] consume_referral: expired referral cookie")
        except Exception:
            pass
        print("[AUTH] consume_referral: returning ref =", code)
        return code
    print("[AUTH] consume_referral: returning ref = None")
    return None
...
def _use_auth_cookies() -> bool:
    raw = (_get_setting("AUTH_USE_COOKIE") or "").strip().lower()
    if raw in {"0", "false", "no", "off"}:
        return False
    if raw in {"1", "true", "yes", "on"}:
        return True
    host = (_request_host() or "").lower()
    app_url = (_get_setting("APP_URL") or "").lower()
    if not host:
        return False
    if "streamlit.app" in host or "streamlit.app" in app_url:
        return False
    return True
...
def _pending_ref_global_set(ref: str | None) -> None:
    store = _load_session_store()
    now = time.time()
    if ref and ref.strip():
        store["_pending_ref_global"] = (now, {"ref": ref.strip()})
    else:
        store.pop("_pending_ref_global", None)
    _save_session_store(store)


def _pending_ref_global_get() -> str | None:
    store = _load_session_store()
    item = store.get("_pending_ref_global")
    if not item:
        return None
    ts, payload = item
    if time.time() - ts > _PENDING_REF_TTL_SECONDS:
...
def _cleanup_pending_ref_email_store() -> None:
    now = time.time()
    expired = [k for k, (ts, _) in _PENDING_REF_BY_EMAIL.items() if now - ts > _PENDING_REF_TTL_SECONDS]
    for key in expired:
        _PENDING_REF_BY_EMAIL.pop(key, None)
        ...
        debug.update(
            {
                ...
                "cookie_error": "disabled",
            }
        )
        st.session_state["_auth_debug"] = debug
        _debug_log("auth_debug cookies disabled")
        return debug
    try:
...
def _load_user_from_cookie(cookies: CookieManager, cfg: dict[str, Any]) -> dict[str, Any] | None:
    if not _use_auth_cookies():
        _debug_log("cookie_load disabled (cookies disabled)")
        return None
...
def _build_login_url(cfg: dict[str, Any], cookies: CookieManager | None) -> str:
    oauth = OAuth2Session(
    ...
    if cookies is not None and _use_auth_cookies():
        try:
            if cookies.ready():
...
def _exchange_code_for_user(
    ...
    if not expected_state and cookies is not None and _use_auth_cookies():
        try:
            if cookies.ready():
    ...
    if not expected_nonce and cookies is not None and _use_auth_cookies():
        try:
            if cookies.ready():
...
def get_current_user(cookies: CookieManager | None = None) -> dict[str, Any] | None:
    ...
    if not _use_auth_cookies():
        ...
        if isinstance(user, dict) and user.get("email"):
            return user
        token = _ensure_session_token(cfg)
        if token:
            store_user = _session_store_get(token)
            if store_user:
...
def _render_home_screen(
    ...
    # OAuth must start in a top-level context, so open in a new tab.
...
def _post_login(user: dict[str, Any]) -> dict[str, Any]:
    ...
    if _post_login_already_done(email):
        return user

    if not _post_login_try_claim(email):
        return user

    print("[AUTH] _post_login executing for", email)

    try:
        params = _get_query_params()
        existing = get_account_by_email_local(email) if email else None
        print(
            "[AUTH] _post_login: existing_before=",
            bool(existing),
            "existing_id=",
            existing.get("id") if existing else None,
        )

        code_any = _pending_ref_global_get()
        if email and code_any:
            _PENDING_REF_BY_EMAIL[email.lower()] = (time.time(), code_any)
            _pending_ref_global_set(None)
            print("[AUTH] mapped pending ref to email:", email, code_any)

        ref_code = _consume_referral_code(params, email=email)
        if existing and (existing.get("referrer_code") or "").strip():
            print("[AUTH] _post_login: existing has referrer_code, skipping apply")
            ref_code = None
        print("[AUTH] _post_login: ref_code_to_apply=", ref_code)

        account = ensure_account(user, referrer_code=ref_code)
        print(
            "[AUTH] ensure_account done:",
            "email=",
            account.get("email") if account else None,
            "referrer_code=",
            account.get("referrer_code") if account else None,
        )
        ...
        _post_login_mark_done(email)
        return user

    except Exception as exc:
        print("[AUTH] _post_login error:", type(exc).__name__, str(exc))
        raise
...
def require_login(force_login: bool = True) -> dict[str, Any] | None:
    ...
    if not oidc_user.get("bypass"):
        st.session_state["_post_login_done"] = True
        st.session_state["_post_login_email"] = oidc_user.get("email")
        try:
            oidc_user = _post_login(oidc_user)
        except Exception as exc:
            _debug_log(f"_post_login failed: {type(exc).__name__} {exc}")
