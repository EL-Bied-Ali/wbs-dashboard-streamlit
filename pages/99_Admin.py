...
from billing_store import (
    ...
    list_events,
    list_referrals,
    recompute_plan_end_from_credits,
    update_account_plan,
)
...
if accounts:
    ...
    if st.button("Apply plan", key="admin_plan_apply"):
        ...
        if ok:
            acc = get_account_by_email_local(selected_email)
            account_id = int(acc["id"]) if acc and acc.get("id") else None
            if account_id:
                did = recompute_plan_end_from_credits(account_id)
                st.info(f"Recompute plan_end from credits for account_id={account_id}: {did}")
            else:
                st.warning("Could not resolve account_id after update.")
...
if user and user.get("email"):
    ...
    if user_account and user_account.get("referral_code"):
        link = f"{_base_url()}/Home?ref={user_account['referral_code']}"
