...
from backup_r2 import run_backup_now, get_backup_stats, list_backups, restore_backup
...
def _cached_backup_stats():
    return get_backup_stats()

@st.cache_data(ttl=60, show_spinner=False)
def _cached_backups():
    return list_backups(limit=50)
...
def _fetch_remote_account_debug(email: str) -> dict:
    base_url = _get_secret("BILLING_API_URL")
    token = _get_secret("BILLING_API_TOKEN")
    if not base_url:
        return {"ok": False, "error": "Missing BILLING_API_URL"}
    headers = {"Accept": "application/json"}
    if token:
        headers["Authorization"] = f"Bearer {token}"
    url = f"{base_url.rstrip('/')}/account?email={email}"
    try:
        req = Request(url, headers=headers, method="GET")
        with urlopen(req, timeout=6) as resp:
            status = getattr(resp, "status", None) or getattr(resp, "code", None)
            body = resp.read().decode("utf-8")
            return {"ok": True, "status": status, "url": url, "body": body[:1000], "token_present": bool(token)}
    except Exception as err:
        return {"ok": False, "error": str(err), "url": url, "token_present": bool(token)}
    return val
...
def _paddle_overlay_widget_html(
    ...
    debug_block = ""
    if is_admin:
        debug_controls = '<button id="paddle-debug-toggle" class="checkout-debug-toggle" type="button">Show debug log</button>'
        debug_block = '<pre id="paddle-debug" class="checkout-debug"></pre>'
...
plan_updated_at_raw = None
plan_updated_at = None
if account and isinstance(account, dict):
    plan_updated_at_raw = account.get("plan_updated_at")
    plan_updated_at = _parse_iso(plan_updated_at_raw if isinstance(plan_updated_at_raw, str) else None)
...
if plan_updated_at_raw:
    updated_label = plan_updated_at.strftime("%Y-%m-%d %H:%M UTC") if plan_updated_at else plan_updated_at_raw
    st.caption(f"Billing sync: updated_at={updated_label}")
...
if is_admin:
    with st.expander("Admin billing diagnostics", expanded=False):
        ...
        # Backup monitoring
        st.markdown("**Backup monitoring**")
        stats = _cached_backup_stats()
        if not stats or not stats.get("configured"):
            msg = (stats or {}).get("error") or "R2 backup not configured."
            st.caption(msg)
        else:
            last_at = stats.get("last_backup_at") or "—"
            last_key = stats.get("last_backup_key") or "—"
            last_size = stats.get("last_backup_size_bytes")
            total_size = stats.get("total_backups_size_bytes", 0) or 0
            total_count = stats.get("total_backups_count", 0) or 0
            keep_limit = stats.get("keep_limit")
            last_restore_at = stats.get("last_restore_at") or "—"
            last_restore_key = stats.get("last_restore_key") or "—"
            last_restore_status = stats.get("last_restore_status") or "—"

            def _fmt_mb(val: int | None) -> str:
                if val is None:
                    return "—"
                return f"{val / (1024 * 1024):.2f} MB"

            free_tier_bytes = 10 * 1024 * 1024 * 1024
            pct_used = (total_size / free_tier_bytes) * 100 if total_size else 0.0

            st.write(
                {
                    "last_backup_at": last_at,
                    "last_backup_key": last_key,
                    "last_backup_size_mb": _fmt_mb(last_size),
                    "total_backups_size_mb": _fmt_mb(total_size),
                    "total_backups_count": total_count,
                    "keep_limit": keep_limit,
                    "free_tier_used_pct_of_10GB": f"{pct_used:.2f}%",
                    "last_restore_at": last_restore_at,
                    "last_restore_key": last_restore_key,
                    "last_restore_status": last_restore_status,
                }
            )
        st.markdown("**Restore from backup (admin)**")
        backups = _cached_backups()
        if not backups:
            st.caption("No backups found or R2 not configured.")
        else:
            options = [
                f"{b.get('key')} ({(b.get('size') or 0)/(1024*1024):.2f} MB)"
                for b in backups
            ]
            key_map = {opt: b.get("key") for opt, b in zip(options, backups)}
            choice = st.selectbox(
            ...
            if st.button("Restore selected backup", key="restore_backup_button"):
                selected_key = key_map.get(choice)
                ok, message, issues = restore_backup(selected_key)
                if ok:
                    st.success(f"Restore applied from {selected_key}")
                    if issues:
                        st.info(f"Issues: {issues}")
                    st.session_state["billing_restore_ts"] = time.time()
                    st.rerun()
                else:
                    st.error(f"Restore failed: {message}")
                    if issues:
                        st.write({"issues": issues})
        st.markdown("**Billing sync diagnostics**")
        col_diag_left, col_diag_right = st.columns(2)
        with col_diag_left:
            if st.button("Force resync + fetch remote", key="billing_force_resync"):
                remote_info = _fetch_remote_account_debug(user_email)
                synced = force_sync_account_from_remote(user_email)
                st.write({"force_sync": synced})
                st.write({"remote_response": remote_info})
                # If we synced successfully, rerun to refresh account/plan_state.
                if synced:
                    st.session_state["billing_force_sync_ts"] = time.time()
                    _rerun()
        with col_diag_right:
            account_row = get_account_row(user_email)
            st.write(
                {
                    "local_plan_status": account_row.get("plan_status") if account_row else None,
                    "local_plan_end": account_row.get("plan_end") if account_row else None,
                    "local_plan_updated_at": account_row.get("plan_updated_at") if account_row else None,
                    "local_account_id": account_row.get("id") if account_row else None,
                }
            )
