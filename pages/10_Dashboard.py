...
if not st.session_state.get("active_project_id"):
    # Fallback to first available project for this owner
    fallback_projects = list_projects(owner_id) if owner_id else []
    if fallback_projects:
        st.session_state["active_project_id"] = fallback_projects[0].get("id")
    else:
...
plan_status = (gate.get("status") or "trialing").lower()
plan_end = gate.get("plan_end")
days_left = gate.get("days_left")
plan_updated_at = account.get("plan_updated_at") if isinstance(account, dict) else None
if plan_status == "trialing" and days_left is not None and days_left <= 3:
...
def _cached_weekly_progress(
    ...
    column_mapping: dict | None,
    today_key: str,
    version: int = WEEKLY_PROGRESS_CACHE_VERSION,
):
    _ = file_key
    _ = version  # bump to invalidate cache when weekly logic changes
    today = date.fromisoformat(today_key)
...
def _render_excel_format_help():
    with st.sidebar.expander("Excel format guide", expanded=False):
        ...
        st.markdown(
            "<div style='font-size:16px;font-weight:800;'>Download the template first to see the required mapping.</div>",
            unsafe_allow_html=True,
        )
        data, name = _excel_template_bytes()
        debug_info = get_demo_template_debug()

        served_label = debug_info.get("served_path") or debug_info.get("base_path")
        week = str(debug_info.get("target_week") or "unknown-week")
        generated_exists = bool(debug_info.get("generated_exists"))
        generated_mtime = debug_info.get("generated_mtime")
        base_mtime = debug_info.get("base_mtime")
        meta = debug_info.get("meta")

        st.sidebar.caption(
            f"Template served: {served_label} • week={week} • generated={generated_exists} "
            f"• gen_mtime={generated_mtime} • base_mtime={base_mtime} • meta={meta}"
        )
        ...
        st.markdown(
            "\n".join(
                [
                    "**Expected structure**",
                    "- Use four sheets like the template: Activity Summary, Ressource Assign. Budgeted, Ressource Assign. Actual, Ressource Assign. Remaining.",
                    "- Activity IDs must match across all sheets, including indentation (copy the spacing style from the template).",
                    "- Activity Summary columns: Activity ID, Activity Name, Activity Status, BL Project Finish (or Planned Finish), Finish (or Forecast Finish), Units % Complete, Variance - BL Project Finish Date, Budgeted Labor Units.",
                    "- Leaf rule: only leaf rows have Activity Name; whenever Activity Name is filled, Activity Status is also filled.",
                    "- Resource sheets columns: Activity ID, Budgeted Units, Spreadsheet Field (Start/Finish optional).",
                    "- Spreadsheet Field values: Cum Budgeted Units, Cum Actual Units, Cum Remaining Early Units.",
                    "- Weekly date columns start on Monday. Budgeted and Actual share the same weeks; Remaining starts exactly one week after the last Actual week (template already aligned).",
                    "",
                    "**Tips**",
                    "- One header row per sheet, no merged cells.",
                    "- Dates can be Excel dates or YYYY-MM-DD.",
                    "- Template regenerates weekly to stay aligned to the current week.",
                ]
            ),
...
# ---------- Helpers ----------
TOOLTIPS = {
    "planned_progress": "Cumulative progress expected at this date based on the baseline plan.",
    "actual_progress": "Cumulative progress actually achieved to date based on reported quantities.",
    ...
    "delay_ahead": "Difference between forecast finish and planned finish. Negative means ahead of schedule.",
    "sv": "Relative schedule variance between earned and planned progress. Positive means ahead, negative means behind.",
    "spi": "Schedule Performance Index. Values above 100% indicate faster-than-planned execution.",
    "weekly_momentum": "Weekly planned, actual, and forecasted progress increments. Use it to detect short-term acceleration or slowdown.",
    "schedule_gap": "Evolution of schedule variance over time. A declining trend signals increasing schedule risk.",
    "activity_mix": "Distribution of activities by execution status at the selected WBS level.",
    "wbs_selector": "Filters all indicators and charts to the selected WBS level.",
...
def _html_title_attr(tip: str | None) -> str:
    if not tip:
        return ""
    ...
    for idx in range(len(week_labels)):
        ...
        if actual_vals[idx] is not None:
            ...
            if planned_ok and actual_ok:
                sv = actual_vals[idx] - planned_vals[idx]
                sv_vals.append(sv)
                sv_text.append(f"{sv:+.2f}%")
                sv_tips.append(
                    f"SV % = Actual % - Planned %<br>Actual: {actual_text[idx]}<br>Planned: {planned_text[idx]}<br>{actual_tips[idx]}<br>{planned_tips[idx]}"
                )
            else:
                sv_vals.append(0)
                sv_text.append("?")
            ...
            if planned_ok and forecast_ok:
                sv = forecast_vals[idx] - planned_vals[idx]
                sv_vals.append(sv)
                sv_text.append(f"{sv:+.2f}%")
                sv_tips.append(
                    f"SV % = Forecast % - Planned %<br>Forecast: {forecast_text[idx]}<br>Planned: {planned_text[idx]}<br>{forecast_tips[idx]}<br>{planned_tips[idx]}"
                )
            else:
                sv_vals.append(0)
                sv_text.append("?")
...
def activities_status_fig(data: dict, error_msg: str | None = None, apply_layout: bool = True):
    ...
    if error_msg:
        ...
        status_tips = {
            "Completed": "Activities fully executed and closed.",
            "In Progress": "Activities currently under execution.",
            "Not Started": "Activities with no recorded progress yet.",
        }
        customdata = [status_tips.get(str(lbl), "") for lbl in labels]
        pie_kwargs.update(
            textinfo="text",
            texttemplate="%{percent:.1%}",
            customdata=customdata,
            hovertemplate="%{label}: %{percent:.2%}<br>%{customdata}<extra></extra>",
        )
...
def compute_dashboard_metrics_from_activity(row: dict, sched_lu: dict):
    if not row:
        return None
    # Tooltips in this section are intentionally definition-only (no formulas / no cell references).
    ...
    if isinstance(sched_val, (int, float)):
        ...
        sched_display = "?"
    schedule_tip = TOOLTIPS["planned_progress"]
    ...
    earned_tip = TOOLTIPS["actual_progress"]
    ...
    if gliss_raw is None or str(gliss_raw).strip() == "":
        ...
        try:
            ...
            delay_display = "?"
            delay_val = None
    delay_tip = TOOLTIPS["delay_ahead"]
            ...
            for col, (role, label, src) in zip(logo_cols, header_logos):
                with col:
                    with st.container(key=f"brand_logo_item_{role}"):
                        st.markdown(
            f'<div class="brand-pill brand-pill--header" title="{html.escape(label, quote=True)} logo">'
