...
.cta-button.is-disabled{
  opacity:0.65; box-shadow:none; pointer-events:none;
...
.project-card{
  ...
  color: var(--text);
  text-decoration:none;
  box-shadow: 0 4px 10px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.03);
...
.project-card::before{
  ...
  background: linear-gradient(180deg, #6dd5ed, #b47cff);
  opacity:0.85;
}
...
.project-card-tool:hover{ border-color: rgba(109,213,237,0.55); color: var(--text); }

/* hover */
.project-card:hover{
  transform: translateY(-4px);
  border-color: rgba(109,213,237,0.45);
  box-shadow: 0 12px 32px rgba(0,0,0,0.45), 0 0 0 1px rgba(109,213,237,0.15);
}

/* title */
.project-name{ font-size:18px; font-weight:700; letter-spacing:0.2px; }
...
.project-action:visited{ color: var(--accent); }
.project-action:hover{ text-decoration: underline; }

/* modal */
.manage-modal-title{ font-size:18px; font-weight:700; color: var(--text); }
...
#st-key-manage_modal_root, #st-key-create_modal_root{
...
div[id^="st-key-create_form_"] .stButton > button:hover{ filter: brightness(1.08); }
...
div[id^="st-key-manage_save_"] .stButton > button:hover{ filter: brightness(1.08); }
...
.project-card.is-disabled{ cursor:not-allowed; opacity:0.7; }
.project-card.is-locked{ cursor:not-allowed; opacity:0.75; }
...
.project-card.is-locked .project-action{ color: var(--muted); }
.project-card.is-disabled:hover{
  transform:none;
  border-color: rgba(148,163,184,0.14);
  box-shadow: 0 4px 10px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.03);
}
.project-meta + .project-meta{ margin-top:2px; }
...
.admin-note{ margin-top:10px; font-size:12px; color: var(--muted); }
...
def _open_create_dialog(
    ...
    def _dialog() -> None:
        with st.container(key="create_modal_root"):
            st.markdown(
                '<div class="manage-modal-title">Create project</div>',
                unsafe_allow_html=True,
            )
            st.markdown(
    ...
    if hasattr(st, "dialog"):
        ...
        def _dialog() -> None:
            with st.container(key="manage_modal_root"):
                st.markdown(
                    f'<div class="manage-modal-title">{html.escape(name)}</div>',
                    unsafe_allow_html=True,
                )
                st.markdown(
                    '<div class="manage-modal-sub">Edit project details and data.</div>',
                    unsafe_allow_html=True,
                )
...
def _render_project_actions(
    ...
    with st.container(key=f"manage_rename_{project_id}"):
        st.markdown(
            '<div class="manage-modal-label">Rename project</div>',
            unsafe_allow_html=True,
        )
    ...
    with st.container(key=f"manage_danger_{project_id}"):
        st.markdown(
            '<div class="manage-modal-label">Danger zone</div>',
            unsafe_allow_html=True,
        )
                        ...
                        if st.button("✔", key=f"confirm_delete_{project_id}", help="Confirm delete"):
                            if delete_project(project_id, owner_id=owner_id):
                                ...
                                st.session_state["project_flash"] = (
                                    f'Project "{current_name}" deleted.'
                                )
                                _clear_query_params()
                                st.rerun()
                            else:
...
def _sort_projects(items: list[dict], sort_mode: str) -> list[dict]:
    ...
    if sort_mode == "Created (newest)":
        return sorted(
            items,
            key=lambda p: _parse_dt(p.get("created_at")) or datetime.min,
            reverse=True,
        )
    return sorted(
        items,
        key=lambda p: _parse_dt(p.get("updated_at")) or datetime.min,
        reverse=True,
    )
...
_render_html(grid_placeholder, f'<div class="project-grid">{"".join(cards)}</div>')
_debug_log("projects: rendered placeholder grid")

# =============================
# VALIDATION + FINAL GRID
# =============================
if validate_excel:
    ...
    for p in projects:
        ...
        expected_key = project_mapping_key(project_id, file_key)
        if expected_key and mapping_key and mapping_key != expected_key:
            status_cache[p.get("id")] = (
                "Needs mapping (Stale)",
                "warn",
                "Mapping is for a different upload.",
            )
        else:
            status_cache[p.get("id")] = (
                "Unchecked",
                "warn",
                "Click “Validate Excel files” to compute readiness.",
            )
...
for p in filtered:
    ...
    detail_line_html = (
        f'<div class="project-meta">{html.escape(status_detail)}</div>' if status_detail else None
    )

    manage_link = (
        f'<div class="project-card-toolbar">'
        f'<a class="project-card-tool" href="?manage={html.escape(project_id)}">Manage</a>'
        f"</div>"
    )
