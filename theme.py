...
/* ============ Section / Carte N2 ============ */
.section-card{
  background:linear-gradient(180deg,#0f1a31,#0b1326); border:1px solid #223355; border-radius:12px;
  padding:8px 10px; margin:4px 0 6px; box-shadow:0 0 0 1px rgba(36,52,83,.35) inset;
...
.n2g-label .badge{padding:3px 10px;font-size:.88rem;font-weight:700;color:#cffafe;background:linear-gradient(180deg,rgba(14,165,233,.18),rgba(14,165,233,.10));border:1px solid rgba(96,165,250,.5);border-radius:999px;transition:transform .15s, box-shadow .15s, border-color .15s}
...
/* Hover ligne N2 (+ overlay clic pleine largeur) */
.n2-grid:hover{position:relative;background:linear-gradient(90deg,rgba(37,99,235,.05) 0%,rgba(37,99,235,.18) 50%,rgba(37,99,235,.05) 100%);background-size:200% 100%;animation:n2RowGlow .9s ease forwards;filter:brightness(1.05);box-shadow:0 0 0 1px rgba(88,113,179,.35) inset;transition:filter .15s, box-shadow .15s}
...
div[class*="st-key-n2_"][class*="__rowbtn"] .stButton button:hover{background:linear-gradient(90deg,rgba(37,99,235,.05),rgba(37,99,235,.18),rgba(37,99,235,.05));background-size:200% 100%;animation:n2RowGlow .9s ease forwards;box-shadow:0 0 0 1px rgba(88,113,179,.35) inset}

/* ============ Tableau N3 (mini-barres HTML optionnelles) ============ */
.table-card{
...
table.neo thead{display:none!important}
...
table.neo tbody tr:hover{background:rgba(148,163,184,.06);transition:background .12s}
...
button[data-testid="stSidebarCollapseButton"]{position:fixed;top:12px;left:12px;z-index:600;width:38px;height:38px;border-radius:10px;opacity:1!important;pointer-events:auto!important;background:rgba(15,23,42,.92)!important;border:1px solid rgba(96,165,250,.55)!important;box-shadow:0 6px 18px rgba(0,0,0,.35)}
...
/* ====== Expander N3 (invisible) ====== */
div[data-testid="stExpander"],div[data-testid="stExpander"]>details,div[data-testid="stExpander"]>details>summary{border:0!important;background:transparent!important;box-shadow:none!important;margin:0!important;padding:0!important}
...
/* ====== Animations bar chart Plotly (REQUIRED) ====== */
...
/* Entrée du frame Plotly (v0/v1) */
details[open] [data-testid="stExpanderDetails"]
  .stElementContainer:has(.n3load.v0) ~ .stElementContainer [data-testid="stFullScreenFrame"],
details[open] [data-testid="stExpanderDetails"]
  .stElementContainer:has(.n3load.v0) ~ .stElementContainer [data-testid="stPlotlyChart"]{
  opacity:0; animation:n3ChartInA .6s cubic-bezier(.22,.61,.36,1) .25s forwards; will-change:opacity,transform;
}
details[open] [data-testid="stExpanderDetails"]
  .stElementContainer:has(.n3load.v1) ~ .stElementContainer [data-testid="stFullScreenFrame"],
details[open] [data-testid="stExpanderDetails"]
  .stElementContainer:has(.n3load.v1) ~ .stElementContainer [data-testid="stPlotlyChart"]{
  opacity:0; animation:n3ChartInB .6s cubic-bezier(.22,.61,.36,1) .25s forwards; will-change:opacity,transform;
}

/* Croissance des barres Plotly (v0/v1) */
details[open] [data-testid="stExpanderDetails"]
  .stElementContainer:has(.n3load.v0) ~ .stElementContainer .main-svg .barlayer{
...
details[open] [data-testid="stExpanderDetails"]
  .stElementContainer:has(.n3load.v1) ~ .stElementContainer .main-svg .barlayer{
  transform-origin:bottom; transform-box:view-box; transform:scaleY(0.001); opacity:0;
  animation:n3GrowB .7s cubic-bezier(.22,.61,.36,1) .2s forwards; will-change:transform,opacity;
}

/* Accessibilité anims N3 */
