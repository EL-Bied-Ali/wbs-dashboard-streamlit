export default {
  async fetch(request, env) {
    ...
    if (path === "/health") {
      return jsonResponse({ ok: true, status: "ready", version: "subuid-2026-01-21-1" });
    }
    ...
    if (secret) {
      ...
      const webhookSubId =
        toStringOrNull(data.subscription_id)
        || toStringOrNull((data.subscription || {}).id);
      let subId = webhookSubId;

      let existingRecord = null;
      if (normalizedEmail) {
        existingRecord = await env.BILLING_KV.get(accountKey(normalizedEmail), { type: "json" });
      }
      if (!existingRecord && accountId) {
        existingRecord = await env.BILLING_KV.get(accountIdKey(accountId), { type: "json" });
      }

      let resolvedViaFetch = false;
      if (!subId) {
        const txnId = toStringOrNull(data.id);
        if (txnId && txnId.startsWith("txn_")) {
          const fetched = await fetchSubscriptionIdFromTxn(env, txnId);
          if (fetched) {
      ...
      const customerId = toStringOrNull(data.customer_id || (data.customer || {}).id);
      const record = {
        ...
        // txn_... (transaction id) for debugging/auditing
        paddle_subscription_id: toStringOrNull(data.id),
        last_event: payload.event_type || payload.alert_name || "unknown",
        updated_at: nowIso(),
      };
      if (customerId) {
        record.paddle_customer_id = customerId;
      }
      const mergedRecord = { ...(existingRecord || {}), ...record };
      const existingSubUid = toStringOrNull(existingRecord?.paddle_subscription_uid);
      const finalSubId = subId || existingSubUid;
      if (finalSubId) {
        mergedRecord.paddle_subscription_uid = finalSubId;
      } else {
        delete mergedRecord.paddle_subscription_uid;
      }
      if (resolvedViaFetch && finalSubId) {
        mergedRecord.paddle_subscription_uid_source = "fetch";
        mergedRecord.paddle_subscription_uid_last_fetched_at = nowIso();
      } else if (webhookSubId && finalSubId) {
        mergedRecord.paddle_subscription_uid_source = "webhook";
        delete mergedRecord.paddle_subscription_uid_last_fetched_at;
      } else {
        delete mergedRecord.paddle_subscription_uid_source;
        delete mergedRecord.paddle_subscription_uid_last_fetched_at;
      }

      if (normalizedEmail) {
        await env.BILLING_KV.put(accountKey(normalizedEmail), JSON.stringify(mergedRecord));
      }
      if (accountId) {
        await env.BILLING_KV.put(accountIdKey(accountId), JSON.stringify(mergedRecord));
      }
...
async function fetchSubscriptionIdFromTxn(env, txnId) {
  const apiToken = env.PADDLE_API_TOKEN || "";
  if (!apiToken || !txnId) {
    return null;
  }
  const envName = String(env.PADDLE_ENV || "sandbox").toLowerCase();
  const base = envName === "production" ? "https://api.paddle.com" : "https://sandbox-api.paddle.com";
  const resp = await fetch(`${base}/transactions/${txnId}`, {
    headers: {
      Authorization: `Bearer ${apiToken}`,
      Accept: "application/json",
    },
  ...
  return toStringOrNull(payload?.data?.subscription_id);
}
