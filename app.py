    ...
    <div class="table-card">
      <div class="table-wrap">
        <table class="neo">
          <thead>
            <tr>
              <th></th><th>Planned</th><th>Forecast</th>
              <th>Schedule</th><th>Earned</th><th>Écart</th><th>Impact</th><th>Glissement</th>
            </tr>
          </thead>
...
def render_barchart(node: dict, chart_key: str | None = None) -> bool:
    ...
    fig=go.Figure()
    # Schedule
    fig.add_bar(name="Schedule %", x=[i - delta for i in idx], y=[v*1.02 for v in schedule],
                width=w, marker=dict(color=c_sched), opacity=1, hoverinfo="skip",
                showlegend=False, cliponaxis=False)
    fig.add_bar(name="Schedule %", x=[i - delta for i in idx], y=schedule, width=w,
                marker=dict(color=f"rgba(96,165,250,{gloss_alpha})", line=dict(color="#93c5fd", width=0.8)),
    ...
    # Earned
    fig.add_bar(name="Earned %", x=[i + delta for i in idx], y=[v*1.02 for v in earned],
                width=w, marker=dict(color=c_earn), opacity=1, hoverinfo="skip",
                showlegend=False, cliponaxis=False)
    fig.add_bar(name="Earned %", x=[i + delta for i in idx], y=earned, width=w,
                marker=dict(color=f"rgba(34,197,94,{gloss_alpha})", line=dict(color="#86efac", width=0.8)),
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="", range=[0, ymax*1.1], ticksuffix="%",
...
def header_level1_grid(label_n1: str, m: dict) -> str:
    ...
    <div class="hero">
      <div class="n1-grid">
        <div class="n1g-label"><span class="dot"></span><span class="title">{label_n1}</span></div>
